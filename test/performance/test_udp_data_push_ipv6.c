...
void send_msg(char *buffer) {
  ...
  jnx_socket_udp_send(s,"::1","9090",buffer,strlen(buffer));
...
void *run_listener(void *args) {
  ...
  jnx_socket *listener = jnx_socket_udp_create(AF_INET6);
...
int main(int argc, char **argv) {
  ...
  JNX_LOG(NULL,"UDP - Received and saved an average of %zuMB per second total %d messages\n",(total_bytes / max_time),count);
  return 0;
}
