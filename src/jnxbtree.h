 ...
 *
 * @parameter order The order of the tree, i.e. the minumum number of records held in a 
 *                  non-root node.
 * @parameter callback The function that takes to keys as void pointers, and returns an
 ...
 *
 * @parameter tree The B-tree to which new record is added
 * @parameter key The key for the record
 * @parameter value The value for the record
 *
 ...
 *
 * @parameter tree The B-tree in which to look up the value for key.
 * @parameter key The key for lookup.
 * 
 ...
 *
 * If the record with key "key" is in the tree, the record is removed and
 * the tree is rebalanced if necessary. If the record with key "key"
 * is not in the tree, the tree is unchanged.
 *
 * @parameter tree The B-tree from which to remove the record.
 * @parameter key The key for which to remove the record.
 *
