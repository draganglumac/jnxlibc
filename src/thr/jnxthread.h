...
#include <pthread.h>
  ...
  /**
   *@fn int32_t jnx_thread_create_disposable(entry_point e,void *args)
   *@param entry_point is the function point32_ter the thread starts with
   *@param args are the arguments to pass the function point32_ter
  ...
  /**
   *@fn int32_t jnx_thread_join(jnx_thread *thr)
   *@param jnx_thread point32_ter to thread object to wait for
   *@param data the exit data from the target thread
   *@brief passing a jnx_thread point32_ter will wait for that thread to complete before unblocking
